// Auto-generated menu data based on the latest sitemap.
// Only links that exist in the current sitemap are retained.
import type { HeaderItem } from '../menu.types'

/**
 * Information architecture (2026 redesign):
 *   Home · Programs · Countries · Solutions · Insights · About  + primary CTA
 *
 * - Programs  → find by pathway type (RBI, CBI, Golden Visa, Skilled, Corporate, Work Permits)
 * - Countries → find by destination (region-grouped → /countries/[slug] overview pages)
 * - Solutions → find by who you are (investor / family / professional / business / entrepreneur)
 *
 * MegaPanel shows a flag if you add `meta.code` (ISO-2) or `meta.iconEmoji`.
 */

// ---------------------------
// Residency (countries)
// ---------------------------
const residencyCountries: HeaderItem[] = [
  {
    label: 'Bulgaria',
    href: '/residency/bulgaria',
    meta: { code: 'BG' },
    submenu: [
      { label: 'AIF Residency', href: '/residency/bulgaria/bulgaria-aif-residency' },
      { label: 'Government Bonds Residency', href: '/residency/bulgaria/bulgaria-government-bonds-residency' },
      { label: 'Real Estate Residency', href: '/residency/bulgaria/bulgaria-real-estate-residency' },
    ],
  },
  {
    label: 'Canada',
    href: '/residency/canada',
    meta: { code: 'CA' },
    submenu: [
      { label: 'Alberta Self Employed farmer', href: '/residency/canada/alberta-self-employed-farmer' },
      { label: 'British Columbia Entrepreneur Base', href: '/residency/canada/british-columbia-entrepreneur-base' },
      { label: 'British Columbia Regional Pilot', href: '/residency/canada/british-columbia-regional-pilot' },
      { label: 'Federal Self Employed', href: '/residency/canada/federal-self-employed' },
      { label: 'Federal Start Up Visa', href: '/residency/canada/federal-start-up-visa' },
      { label: 'Manitoba Entrepreneur', href: '/residency/canada/manitoba-entrepreneur' },
      { label: 'New Brunswick Business-immigration', href: '/residency/canada/new-brunswick-business-immigration' },
      { label: 'New Brunswick Entrepreneur Program', href: '/residency/canada/new-brunswick-entrepreneur-program' },
      { label: 'Newfoundland Entrepreneur', href: '/residency/canada/newfoundland-entrepreneur' },
      { label: 'Newfoundland International Entrepreneur', href: '/residency/canada/newfoundland-international-entrepreneur' },
      { label: 'Northwest Territories Business', href: '/residency/canada/northwest-territories-business' },
      { label: 'Nova scotia Business Immigration', href: '/residency/canada/nova-scotia-business-immigration' },
      { label: 'Nova Scotia Entrepreneur Stream', href: '/residency/canada/nova-scotia-entrepreneur-stream' },
      { label: 'Ontario Entrepreneur', href: '/residency/canada/ontario-entrepreneur' },
      { label: 'Pei Work Permit', href: '/residency/canada/pei-work-permit' },
      { label: 'SaskatChewan Entrepreneur', href: '/residency/canada/saskatchewan-entrepreneur' },
      { label: 'saskatchewan Farm Owner', href: '/residency/canada/saskatchewan-farm-owner' },
      { label: 'Canada Startup Visa', href: '/residency/canada/canada-start-up-visa' },
      { label: 'Yukon Business', href: '/residency/canada/yukon-business' },
    ],
  },
  {
    label: 'Curacao',
    href: '/residency/curacao',
    meta: { code: 'CW' },
    submenu: [
      { label: '3-Year Active Investor', href: '/residency/curacao/3-year-active-investor' },
      { label: 'Indefinite Investor Residency', href: '/residency/curacao/indefinite-investor-residency' },
    ],
  },
  {
    label: 'Cyprus',
    href: '/residency/cyprus',
    meta: { code: 'CY' },
    submenu: [
      { label: 'Business Investment', href: '/residency/cyprus/business-investment' },
      { label: 'Commercial Property', href: '/residency/cyprus/commercial-property' },
      { label: 'Fund Investment', href: '/residency/cyprus/fund-investment' },
      { label: 'Residential Property', href: '/residency/cyprus/residential-property' },
    ],
  },
  {
    label: 'Greece',
    href: '/residency/greece',
    meta: { code: 'GR' },
    submenu: [
      { label: 'Capital Investment', href: '/residency/greece/greece-capital-investment' },
      { label: 'Real Estate Investment', href: '/residency/greece/greece-real-estate-investment' },
    ],
  },
  {
    label: 'Hong Kong',
    href: '/residency/hong-kong',
    meta: { code: 'HK' },
    submenu: [
      { label: 'HK Business Investment', href: '/residency/hong-kong/hk-business-investment' },
      { label: 'HK Fund Investment', href: '/residency/hong-kong/hk-fund-investment' },
      { label: 'HK Property Investment', href: '/residency/hong-kong/hk-property-investment' },
      { label: 'HK Securities Investment', href: '/residency/hong-kong/hk-securities-investment' },
    ],
  },
  {
    label: 'Hungary',
    href: '/residency/hungary',
    meta: { code: 'HU' },
    submenu: [
      { label: 'Donation Public Trust', href: '/residency/hungary/hungary-donation-public-trust' },
      { label: 'Real Estate Fund', href: '/residency/hungary/hungary-real-estate-fund' },
    ],
  },
  {
    label: 'Latvia',
    href: '/residency/latvia',
    meta: { code: 'LV' },
    submenu: [
      { label: 'Bank Deposit', href: '/residency/latvia/latvia-bank-deposit' },
      { label: 'Business Investment', href: '/residency/latvia/latvia-business-investment' },
      { label: 'Government Bonds', href: '/residency/latvia/latvia-government-bonds' },
      { label: 'Real Estate Investment', href: '/residency/latvia/latvia-real-estate-investment' },
    ],
  },
  {
    label: 'Malaysia',
    href: '/residency/malaysia',
    meta: { code: 'MY' },
    submenu: [
      { label: 'Malaysia Mm2h Property', href: '/residency/malaysia/malaysia-mm2h-property' },
      { label: 'Malaysia Mm2h Silver', href: '/residency/malaysia/malaysia-mm2h-silver' },
      { label: 'Malaysia Mm2h Gold', href: '/residency/malaysia/malaysia-mm2h-gold' },
    ],
  },
  {
    label: 'Malta',
    href: '/residency/malta',
    meta: { code: 'MT' },
    submenu: [
      { label: 'Government Contribution', href: '/residency/malta/malta-government-contribution' },
      { label: 'Property Lease Residency', href: '/residency/malta/malta-property-lease-residency' },
      { label: 'Property Purchase', href: '/residency/malta/malta-property-purchase' },
    ],
  },
  {
    label: 'Mauritius',
    href: '/residency/mauritius',
    meta: { code: 'MU' },
    submenu: [
      { label: 'Mauritius Business Investment', href: '/residency/mauritius/mauritius-business-investment' },
      { label: 'Mauritius Strategic Fund Investment', href: '/residency/mauritius/mauritius-strategic-fund-investment' },
      { label: 'Mauritius Real Estate Investment', href: '/residency/mauritius/mauritius-real-estate-investment' },
      { label: 'Mauritius Retirement Transfer', href: '/residency/mauritius/mauritius-retirement-transfer' },
    ],
  },
  {
    label: 'Monaco',
    href: '/residency/monaco',
    meta: { code: 'MC' },
    submenu: [
      { label: 'Bank Deposit', href: '/residency/monaco/monaco-residency-bank-deposit' },
      { label: 'Property Investment', href: '/residency/monaco/monaco-residency-property-investment' },
    ],
  },
  {
    label: 'New Zealand',
    href: '/residency/new-zealand',
    meta: { code: 'NZ' },
    submenu: [
      { label: 'Active Investor Plus Balanced Category', href: '/residency/new-zealand/active-investor-plus-balanced-category' },
      { label: 'Active Investor Plus Growth Category', href: '/residency/new-zealand/active-investor-plus-growth-category' },
	  { label: 'Business Investor Visa', href: '/residency/new-zealand/business-investor-visa' },
    ],
  },
  {
    label: 'Panama',
    href: '/residency/panama',
    meta: { code: 'PA' },
    submenu: [
      { label: 'Panama RBI via Real Estate', href: '/residency/panama/panama-residency-real-estate' },
      { label: 'Panama Residency Bank Deposit', href: '/residency/panama/panama-residency-bank-deposit' },
      { label: 'Panama RBI Stock Market', href: '/residency/panama/panama-residency-stock-market' },
    ],
  },
  {
    label: 'Portugal',
    href: '/residency/portugal',
    meta: { code: 'PT' },
    submenu: [
      { label: 'Business Investment', href: '/residency/portugal/portugal-business-investment' },
      { label: 'Capital Transfer', href: '/residency/portugal/portugal-capital-transfer' },
    ],
  },
  {
    label: 'Singapore',
    href: '/residency/singapore',
    meta: { code: 'SG' },
    submenu: [
      { label: 'GIP Business Investment', href: '/residency/singapore/singapore-gip-business-investment' },
      { label: 'GIP Fund Investment', href: '/residency/singapore/singapore-gip-fund-investment' },
      { label: 'GIP SFO Residency', href: '/residency/singapore/singapore-gip-sfo-residency' },
    ],
  },
  {
    label: 'Switzerland',
    href: '/residency/switzerland',
    meta: { code: 'CH' },
    submenu: [
      { label: 'Business Investment', href: '/residency/switzerland/switzerland-business-investment' },
      { label: 'Lump Sum Tax', href: '/residency/switzerland/switzerland-lump-sum-tax' },
    ],
  },
  {
    label: 'UAE',
    href: '/residency/uae',
    meta: { code: 'AE' },
    submenu: [
      { label: 'Real Estate', href: '/residency/uae/uae-golden-visa' },
      { label: 'Specialized Talent', href: '/residency/uae/uae-specialized-talent' },
    ],
  },
  {
    label: 'Uruguay',
    href: '/residency/uruguay',
    meta: { code: 'UY' },
    submenu: [
      { label: 'Uruguay Business Investment', href: '/residency/uruguay/uruguay-business-investment' },
      { label: 'Uruguay Real Estate Residency', href: '/residency/uruguay/uruguay-real-estate-residency' },
    ],
  },
  {
    label: 'USA',
    href: '/residency/usa',
    meta: { code: 'US' },
    submenu: [
      { label: 'EB-5 Non-Targeted Employment Area', href: '/residency/usa/eb5-non-targeted-employment-area' },
      { label: 'EB-5 Targeted Employment Area', href: '/residency/usa/eb5-targeted-employment-area' },
    ],
  },
];

// ---------------------------
// Citizenship (countries)
// ---------------------------
const citizenshipCountries: HeaderItem[] = [
  {
    label: 'Antigua & Barbuda',
    href: '/citizenship/antigua-barbuda',
    meta: { code: 'AG' },
    submenu: [
      { label: 'Business Investment', href: '/citizenship/antigua-barbuda/business-investment' },
      { label: 'National Development Fund', href: '/citizenship/antigua-barbuda/national-development-fund' },
      { label: 'Real Estate', href: '/citizenship/antigua-barbuda/real-estate' },
    ],
  },
  {
    label: 'Dominica',
    href: '/citizenship/dominica',
    meta: { code: 'DM' },
    submenu: [
      { label: 'Real Estate', href: '/citizenship/dominica/real-estate' },
      { label: 'Economic Diversification Fund', href: '/citizenship/dominica/economic-diversification-fund' },
    ],
  },
  {
    label: 'Egypt',
    href: '/citizenship/egypt',
    meta: { code: 'EG' },
    badge: { text: 'Fast Track', tone: 'info' },
    submenu: [
      { label: 'Bank Deposit', href: '/citizenship/egypt/bank-deposit' },
      { label: 'Business Investment', href: '/citizenship/egypt/business-investment' },
      { label: 'Donation', href: '/citizenship/egypt/donation' },
      { label: 'Real Estate', href: '/citizenship/egypt/real-estate' },
    ],
  },
  {
    label: 'Grenada',
    href: '/citizenship/grenada',
    meta: { code: 'GD' },
    submenu: [
      { label: 'Real Estate', href: '/citizenship/grenada/real-estate' },
      { label: 'National Transformation Fund', href: '/citizenship/grenada/national-transformation-fund' },
    ],
  },
  {
    label: 'Nauru',
    href: '/citizenship/nauru',
    meta: { code: 'NR' },
    submenu: [
      { label: 'Investment', href: '/citizenship/nauru/investment' },
    ],
  },
  {
    label: 'Saint Lucia',
    href: '/citizenship/saint-lucia',
    meta: { code: 'LC' },
    submenu: [
      { label: 'National Economic Fund', href: '/citizenship/saint-lucia/national-economic-fund' },
      { label: 'Real Estate', href: '/citizenship/saint-lucia/real-estate' },
    ],
  },
  {
    label: 'Saotome',
    href: '/citizenship/saotome',
    meta: { code: 'ST' },
    submenu: [
      { label: 'NTF', href: '/citizenship/saotome/ntf' },
    ],
  },
  {
    label: 'Saint kitts',
    href: '/citizenship/saintkitts',
    meta: { code: 'KN' },
    submenu: [
      { label: 'Approved Public Benefit Project', href: '/citizenship/saintkitts/approved-public-benefit-project' },
      { label: 'Real Estate', href: '/citizenship/saintkitts/real-estate' },
      { label: 'Sustainable Island State Contribution', href: '/citizenship/saintkitts/sustainable-island-state-contribution' },
    ],
  },
  {
    label: 'Turkey',
    href: '/citizenship/turkey',
    meta: { code: 'TR' },
    submenu: [
      { label: 'Bank Deposit', href: '/citizenship/turkey/bank-deposit' },
      { label: 'Business Investment', href: '/citizenship/turkey/business-investment' },
      { label: 'Fund Investment', href: '/citizenship/turkey/fund-investment' },
      { label: 'Government Bonds', href: '/citizenship/turkey/government-bonds' },
      { label: 'Job Creation', href: '/citizenship/turkey/job-creation' },
      { label: 'Real Estate', href: '/citizenship/turkey/real-estate' },
    ],
  },
  {
    label: 'Vanuatu',
    href: '/citizenship/vanuatu',
    meta: { code: 'VU' },
    submenu: [
      { label: 'VDSP Donation', href: '/citizenship/vanuatu/vdsp-donation' },
    ],
  },
];

// ---------------------------
// Corporate (countries)
// ---------------------------
const corporateCountries: HeaderItem[] = [
  {
    label: 'Canada',
    href: '/corporate/canada',
    meta: { code: 'CA' },
    submenu: [
      { label: 'Intra Company Transfer', href: '/corporate/canada/intra-company-transfer' },
    ],
  },
  {
    label: 'Cyprus',
    href: '/corporate/cyprus',
    meta: { code: 'CY' },
    submenu: [
      { label: 'Company Setup', href: '/corporate/cyprus/company-setup' },
    ],
  },
  {
    label: 'Portugal',
    href: '/corporate/portugal',
    meta: { code: 'PT' },
    submenu: [
      { label: 'Portugal D2 Visa', href: '/corporate/portugal/portugal-d2-visa' },
    ],
  },
  {
    label: 'Spain',
    href: '/corporate/spain',
    meta: { code: 'ES' },
    submenu: [
      { label: 'Entrepreneur Company Formation', href: '/corporate/spain/entrepreneur-company-formation' },
    ],
  },
  {
    label: 'UAE',
    href: '/corporate/uae',
    meta: { code: 'AE' },
    submenu: [
      { label: 'Dubai Freezone Visa', href: '/corporate/uae/dubai-freezone-visa' },
      { label: 'Dubai Investor Visa', href: '/corporate/uae/dubai-investor-visa' },
      { label: 'Dubai Mainland Employment Visa', href: '/corporate/uae/dubai-mainland-employment-visa' },
    ],
  },
  {
    label: 'United Kingdom',
    href: '/corporate/united-kingdom',
    meta: { code: 'GB' },
    submenu: [
      { label: 'Expansion Worker Visa', href: '/corporate/united-kingdom/expansion-worker-visa' },
      { label: 'Self Sponsorship Visa', href: '/corporate/united-kingdom/self-sponsorship-visa' },
    ],
  },
  {
    label: 'USA',
    href: '/corporate/usa',
    meta: { code: 'US' },
    submenu: [
      { label: 'L1 Corporate Transfer Visa', href: '/corporate/usa/l1-corporate-transfer-visa' },
      { label: 'O1 Entrepreneur Visa', href: '/corporate/usa/o1-entrepreneur-visa' },
    ],
  },
];

// ---------------------------
// Skilled (countries)
// ---------------------------
const skilledCountries: HeaderItem[] = [
  {
    label: 'Australia',
    href: '/skilled/australia',
    meta: { code: 'AU' },
    submenu: [
      { label: 'Employer Nomination Scheme-186', href: '/skilled/australia/employer-nomination-scheme-186' },
      { label: 'Global Talent Visa 858', href: '/skilled/australia/global-talent-visa-858' },
      { label: 'Regional Sponsored Migration 187', href: '/skilled/australia/regional-sponsored-migration-187' },
      { label: 'Skilled Independent 189', href: '/skilled/australia/skilled-independent-189' },
      { label: 'Skilled Nominated 190', href: '/skilled/australia/skilled-nominated-190' },
      { label: 'Skilled Work Regional 491', href: '/skilled/australia/skilled-work-regional-491' },
    ],
  },
  {
    label: 'Canada',
    href: '/skilled/canada',
    meta: { code: 'CA' },
    submenu: [
      { label: 'Canadian Experience Class', href: '/skilled/canada/canadian-experience-class' },
      { label: 'Express Entry', href: '/skilled/canada/express-entry' },
      { label: 'Federal Skilled Trades', href: '/skilled/canada/federal-skilled-trades' },
      { label: 'Global Talent Stream', href: '/skilled/canada/global-talent-stream' },
      { label: 'Provincial Nominee Program', href: '/skilled/canada/provincial-nominee-program' },
    ],
  },
  {
    label: 'Germany',
    href: '/skilled/germany',
    meta: { code: 'DE' },
    submenu: [
      { label: 'Germany Job Seeker Visa', href: '/skilled/germany/germany-job-seeker-visa' },
    ],
  },
  {
    label: 'Italy',
    href: '/skilled/italy',
    meta: { code: 'IT' },
    submenu: [
      { label: 'Italy Digital Nomad Visa', href: '/skilled/italy/italy-digital-nomad-visa' },
    ],
  },
  {
    label: 'New Zealand',
    href: '/skilled/new-zealand',
    meta: { code: 'NZ' },
    submenu: [
      { label: 'Green List - Tier 1', href: '/skilled/new-zealand/green-list-straight-to-residence-visa' },
      { label: 'Green List - Tier 2', href: '/skilled/new-zealand/green-list-work-to-residence-visa' },
    ],
  },
  {
    label: 'Spain',
    href: '/skilled/spain',
    meta: { code: 'ES' },
    submenu: [
      { label: 'Spain Digital Nomad Visa', href: '/skilled/spain/spain-digital-nomad-visa' },
    ],
  },
  {
    label: 'United Kingdom',
    href: '/skilled/united-kingdom',
    meta: { code: 'GB' },
    submenu: [
      { label: 'UK Global Talent Visa', href: '/skilled/united-kingdom/uk-global-talent-visa' },
    ],
  },
  {
    label: 'USA',
    href: '/skilled/usa',
    meta: { code: 'US' },
    submenu: [
      { label: 'EB-1A Extraordinary Ability', href: '/skilled/usa/eb1a-extraordinary-ability' },
      { label: 'EB-1B Outstanding Professors', href: '/skilled/usa/eb1b-outstanding-professors' },
      { label: 'EB-1C Executive Manager', href: '/skilled/usa/eb1c-executive-manager' },
      { label: 'EB-2 National Interest Waiver', href: '/skilled/usa/eb2-national-interest-waiver' },
      { label: 'EB-3 Employment Based Green Card', href: '/skilled/usa/eb3-employment-based-green-card' },
      { label: 'H1-B Specialty Occupation', href: '/skilled/usa/h1b-specialty-occupation' },
      { label: 'J1 Exchange Visitor Program', href: '/skilled/usa/j1-exchange-visitor-program' },
    ],
  },
];

// ---------------------------
// Work permits (permit advisory; not job placement)
// ---------------------------
const workPermitCountries: HeaderItem[] = [
  {
    label: 'Canada',
    href: '/work-permits?country=canada',
    description: 'GTS, employer-specific work permits, LMIA and ICT direction',
    meta: { code: 'CA' },
  },
  {
    label: 'United States',
    href: '/work-permits?country=usa',
    description: 'H-1B, L-1, O-1, J-1 and sponsored work direction',
    meta: { code: 'US' },
  },
  {
    label: 'United Kingdom',
    href: '/work-permits?country=united-kingdom',
    description: 'Skilled Worker, Global Talent and expansion worker planning',
    meta: { code: 'GB' },
  },
  {
    label: 'Australia',
    href: '/work-permits?country=australia',
    description: 'Employer nomination, points-tested skilled and regional options',
    meta: { code: 'AU' },
  },
  {
    label: 'Germany',
    href: '/work-permits?country=germany',
    description: 'Opportunity Card, EU Blue Card and skilled worker residence',
    meta: { code: 'DE' },
  },
  {
    label: 'UAE',
    href: '/work-permits?country=uae',
    description: 'Mainland, freezone, employment and professional mobility',
    meta: { code: 'AE' },
  },
  {
    label: 'Portugal',
    href: '/work-permits?country=portugal',
    description: 'Work residence, D2 direction and highly qualified activity',
    meta: { code: 'PT' },
  },
  {
    label: 'Spain',
    href: '/work-permits?country=spain',
    description: 'Digital nomad, highly qualified and entrepreneur direction',
    meta: { code: 'ES' },
  },
];

// Golden Visa — the headline residency-by-investment routes (subset of residency).
const goldenVisaCountries: HeaderItem[] = [
  { label: 'Portugal', href: '/residency/portugal', meta: { code: 'PT' } },
  { label: 'Greece', href: '/residency/greece', meta: { code: 'GR' } },
  { label: 'UAE', href: '/residency/uae', meta: { code: 'AE' } },
  { label: 'Malta', href: '/residency/malta', meta: { code: 'MT' } },
  { label: 'Cyprus', href: '/residency/cyprus', meta: { code: 'CY' } },
  { label: 'Hungary', href: '/residency/hungary', meta: { code: 'HU' } },
  { label: 'Latvia', href: '/residency/latvia', meta: { code: 'LV' } },
  { label: 'Mauritius', href: '/residency/mauritius', meta: { code: 'MU' } },
];

// ---------------------------
// Countries (region-grouped → unified /countries/[slug] overview pages)
// ---------------------------
const c = (label: string, slug: string, code: string): HeaderItem => ({
  label,
  href: `/countries/${slug}`,
  meta: { code },
});

// Flat, searchable A–Z list (region grouping lives on the /countries page).
const allCountries: HeaderItem[] = [
  c('Antigua & Barbuda', 'antigua-barbuda', 'AG'),
  c('Australia', 'australia', 'AU'),
  c('Bulgaria', 'bulgaria', 'BG'),
  c('Canada', 'canada', 'CA'),
  c('Curaçao', 'curacao', 'CW'),
  c('Cyprus', 'cyprus', 'CY'),
  c('Dominica', 'dominica', 'DM'),
  c('Egypt', 'egypt', 'EG'),
  c('Germany', 'germany', 'DE'),
  c('Greece', 'greece', 'GR'),
  c('Grenada', 'grenada', 'GD'),
  c('Hong Kong', 'hong-kong', 'HK'),
  c('Hungary', 'hungary', 'HU'),
  c('Italy', 'italy', 'IT'),
  c('Latvia', 'latvia', 'LV'),
  c('Malaysia', 'malaysia', 'MY'),
  c('Malta', 'malta', 'MT'),
  c('Mauritius', 'mauritius', 'MU'),
  c('Monaco', 'monaco', 'MC'),
  c('Nauru', 'nauru', 'NR'),
  c('New Zealand', 'new-zealand', 'NZ'),
  c('Panama', 'panama', 'PA'),
  c('Portugal', 'portugal', 'PT'),
  c('Saint Kitts & Nevis', 'saintkitts', 'KN'),
  c('Saint Lucia', 'saint-lucia', 'LC'),
  c('São Tomé & Príncipe', 'saotome', 'ST'),
  c('Singapore', 'singapore', 'SG'),
  c('Spain', 'spain', 'ES'),
  c('Switzerland', 'switzerland', 'CH'),
  c('Turkey', 'turkey', 'TR'),
  c('UAE', 'uae', 'AE'),
  c('United Kingdom', 'united-kingdom', 'GB'),
  c('United States', 'usa', 'US'),
  c('Uruguay', 'uruguay', 'UY'),
  c('Vanuatu', 'vanuatu', 'VU'),
];

// ---------------------------
// Solutions (audience-led segment pages)
// ---------------------------
const solutionsItems: HeaderItem[] = [
  {
    label: 'For Investors',
    href: '/for-investors',
    description: 'Residency & citizenship by investment, golden visas',
    meta: { iconEmoji: '💼' },
  },
  {
    label: 'For Families',
    href: '/for-families',
    description: 'Family-inclusive routes, schooling and a second passport',
    meta: { iconEmoji: '👨‍👩‍👧' },
  },
  {
    label: 'For Professionals',
    href: '/for-professionals',
    description: 'Points-based PR and skilled work visas',
    meta: { iconEmoji: '🎓' },
  },
  {
    label: 'For Businesses',
    href: '/for-businesses',
    description: 'Corporate mobility, ICT and market entry',
    meta: { iconEmoji: '🏢' },
  },
  {
    label: 'For Entrepreneurs',
    href: '/for-entrepreneurs',
    description: 'Start-up, entrepreneur and self-sponsorship routes',
    meta: { iconEmoji: '🚀' },
  },
];

// ---------------------------
// Insights
// ---------------------------
const insightsItems: HeaderItem[] = [
  { label: 'Blog', href: '/blog', description: 'Guides and analysis from our advisors', meta: { iconEmoji: '📝' } },
  { label: 'Articles', href: '/articles', description: 'In-depth programme and policy articles', meta: { iconEmoji: '📰' } },
  { label: 'News', href: '/news', description: 'Latest immigration updates', meta: { iconEmoji: '🗞️' } },
  { label: 'Media', href: '/media', description: 'XIPHIAS in the press', meta: { iconEmoji: '🎥' } },
  { label: 'Guides', href: '/guide', description: 'Step-by-step migration guides', meta: { iconEmoji: '📘' } },
  { label: 'Events', href: '/events', description: 'Seminars and investor meetups', meta: { iconEmoji: '📅' } },
];

// ---------------------------
// About
// ---------------------------
const aboutItems: HeaderItem[] = [
  { label: 'Company', href: '/about', description: 'Who we are and how we work', meta: { iconEmoji: '🏛️' } },
  { label: 'Team & Leadership', href: '/teams', description: 'The advisors behind your case', meta: { iconEmoji: '👥' } },
  { label: 'Office Locations', href: '/contact', description: 'Reach us across our offices', meta: { iconEmoji: '📍' } },
  { label: 'Awards', href: '/awards', description: 'Recognition and accreditations', meta: { iconEmoji: '🏆' } },
  { label: 'Reviews', href: '/reviews', description: 'What our clients say', meta: { iconEmoji: '⭐' } },
  { label: 'Careers', href: '/careers', description: 'Join the XIPHIAS team', meta: { iconEmoji: '💡' } },
];

// ---------------------------
// Tools (XIA suite + planning tools) — a flagship section
// ---------------------------
const toolsItems: HeaderItem[] = [
  { label: 'XIA Intelligence', href: '/xia-intelligence', description: 'AI route scoring across every pathway', meta: { iconEmoji: '🧠' } },
  { label: 'Eligibility Check', href: '/eligibility', description: 'Screen your profile in minutes', meta: { iconEmoji: '✅' } },
  { label: 'Cost Estimator', href: '/cost-estimator', description: 'Fees, due diligence & dependants', meta: { iconEmoji: '🧮' } },
  { label: 'Compare Programmes', href: '/compare-programs', description: 'Weigh routes side by side', meta: { iconEmoji: '⚖️' } },
  { label: 'Programme Index', href: '/xiphias-program-index', description: 'Benchmark every programme', meta: { iconEmoji: '📊' } },
  { label: 'Passport Index', href: '/passport-index', description: 'Global passport strength & mobility', meta: { iconEmoji: '🛂' } },
  { label: 'X-Hub Portal', href: '/x-hub', description: 'Your application dashboard', meta: { iconEmoji: '🗂️' } },
];

// ---------------------------
// Top-level header menu (2026 IA)
// ---------------------------
export const headerMenu: HeaderItem[] = [
  { label: 'Home', href: '/' },

  // Programs — find by pathway type
  {
    label: 'Programs',
    href: '/xiphias-program-index',
    badge: { text: 'Popular', tone: 'success' },
    submenu: [
      { label: 'Residency by Investment', href: '/residency', submenu: residencyCountries },
      { label: 'Citizenship by Investment', href: '/citizenship', submenu: citizenshipCountries },
      { label: 'Golden Visa', href: '/golden-visa', submenu: goldenVisaCountries },
      { label: 'Skilled Migration', href: '/skilled', submenu: skilledCountries },
      { label: 'Corporate & Business', href: '/corporate', submenu: corporateCountries },
      { label: 'Work Permits', href: '/work-permits', badge: { text: 'Resume Review', tone: 'info' }, submenu: workPermitCountries },
    ],
  },

  // Countries — find by destination
  {
    label: 'Countries',
    href: '/countries',
    submenu: allCountries,
  },

  // Solutions — find by who you are
  {
    label: 'Solutions',
    href: '/for-investors',
    submenu: solutionsItems,
  },

  // Tools — XIA suite + planning tools (flagship)
  {
    label: 'Tools',
    href: '/xia-intelligence',
    badge: { text: 'XIA', tone: 'info' },
    submenu: toolsItems,
  },

  // Insights
  {
    label: 'Insights',
    href: '/insights',
    submenu: insightsItems,
  },

  // About
  {
    label: 'About',
    href: '/about',
    submenu: aboutItems,
  },
];
