...
export default function WhatWeProvide({
  ...
  const renderedItems = imageOverrides
    ? items.map((item) => ({ ...item, img: imageOverrides[item.t] ?? item.img }))
    : items;
  ...
  return (
    <section data-tone="dark" className="relative isolate px-6 py-28 text-[#eef3fb] sm:px-12 lg:px-20" style={{ background: NAVY }}>
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <h2 className={`${serifClass} mt-5 max-w-3xl text-[clamp(2.2rem,4.6vw,3.8rem)] font-medium leading-[1.04]`}>End-to-End Immigration <span className="italic" style={{ color: GOLD }}>Services in Dubai</span></h2>
        <div className="mt-16 flex flex-col gap-24">
          {renderedItems.map((item, i) => <Row key={item.t} item={item} i={i} serifClass={serifClass} />)}
        </div>
      </div>
    </section>
  );
