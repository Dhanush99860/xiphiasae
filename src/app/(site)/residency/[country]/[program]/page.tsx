...
export async function generateStaticParams() {
  return getResidencyCountrySlugs().flatMap((c) => getResidencyPrograms(c).map((p) => ({ country: c, program: p.programSlug })));
}
...
export async function generateMetadata(props: { params: Promise<{ country: string; program: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    const rawD = (m.tagline as string | undefined) ?? `${m.title} — ${m.country} investor residency programme. Arranged end-to-end by XIPHIAS, Dubai.`;
...
export default async function ProgramPage(props: { params: Promise<{ country: string; program: string }> }) {
  ...
  const p = getResidencyPrograms(country).find((x) => x.programSlug === program) as (Record<string, any> & { title: string; country: string; countrySlug: string }) | undefined;
