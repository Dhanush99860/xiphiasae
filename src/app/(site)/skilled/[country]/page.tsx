...
export async function generateMetadata(props: { params: Promise<{ country: string }> }): Promise<Metadata> {
  ...
  const genT = `${cn} Skilled Migration & PR Visa | XIPHIAS`;
  const title = seo?.title ?? (genT.length <= 60 ? genT : `${cn} Skilled Visa & PR | XIPHIAS`);
  const rawD = (m.summary as string | undefined) ?? `${cn} skilled migration & permanent residence — Express Entry, points-based & employer-sponsored routes. XIPHIAS Dubai.`;
